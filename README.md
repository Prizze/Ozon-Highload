...
1. После введенного запроса необходимо: провести токенизацию (разделить слова), привести все к нижнему регистру, отфильтровать стоп-слова, провести лемматизацию токенов. То есть задача - нормализовать входные данные.
2. Первый слой ранжирования. Здесь нужно отобрать "ближайших" кандидатов, учитывая их текстовую релевантность с запросом. Применим TF-IDF и несколько тысяч товаров с наибольшими оценками попадут на следующий этап.
3. Второй слой ранжирования. Здесь можно учесть дополнительные факторы для сортировки отдачи товаров. Будем учитывать количество заказов у этого товара, средний рейтинг товара и количество просмотров  (значение из соответствующего счетчика). Каждый фактор имеет свой вес, например, количество заказов важнее количества просмотров.
4. Получаем финальную оценку товаров и сортируем их.
...
- признаков товаров.
1. Сначала нужно векторизировать пользователя. Собираем его поведение: последние просмотры, любимые категории, купленные товары. Формируем `user_vector` для `User_features`. Берем все `product_vector` и формируем средневзвешанный `user_vector` с учетом весов.
2. Выбор кандидатов для рекомендаций. При необходимости выдачи рекомендаций берем `user_vector` и вычисляем векторную близость с каждым `product_vector`. Отбираются top-N самых "близких" кандидатов.
3. Аналогично поиску учитываем дополнительные факторы.
4. Получаем финальную оценку товаров и сортируем их.
