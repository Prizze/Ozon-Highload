...
- признаков товаров.
1. Сначала нужно векторизировать пользователя. Собираем его действия: последние просмотры, любимые категории, купленные товары. Формируем `user_vector` для `User_features`. Берем все `product_vector` и формируем средневзвешанный `user_vector` с учетом весов.
