...
- Москва (30% нагрузки)
- Санкт-Петербург (27% нагрузки) 
- Новосибирск (25% нагрузки)
- Ростов-на-Дону (18% нагрузки)

### Схема балансировки

#### DNS балансировка
DNS-балансировка хорошо работает в масштабах континентов/стран. Поэтому выбор DNS-балансировки не существенен.

#### Anycast балансировка
Для Anycast балансировки планируется использование BGP Anycast, что обеспечивает маршрутизацию трафика на основе оптимальных сетевых путей. Данная схема применяется внутри регионов для достижения минимальной задержки и высокой отказоустойчивости (в случае выхода из строя одного ДЦ трафик перенаправится в другой ближайший ДЦ). Все ДЦ представлены одним IP-адресом, интернет-провайдер выберет ближайший на основе BGP.
